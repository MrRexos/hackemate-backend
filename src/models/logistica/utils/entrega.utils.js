...
/**
 * Extreu dia i nom canonicals per agrupar (mateix dia calendari + mateix nom client).
 * @param {Pedido} pedido
 * @returns {{ diaKey: string, nomKey: string, clau: string }}
 */
...
/**
 * Agrupa un array de {@link Pedido} en {@link Entrega}: una entrega per cada parell únic (dia, nom).
 * L’adreça es pren de la primera línia del grup que en tingui una; el volum es recalcula a la construcció d’`Entrega`.
 *
...
export function agrupaPedidosEnEntregues(pedidos, options = {}) {
  ...
  for (const [, grup] of grups) {
    const primer = grup[0];
    const { diaKey, nomKey } = clausAgrupacioPedido(primer);
    const adreca =
    ...
    const identificador =
      diaKey || nomKey ? `${diaKey}__${nomKey}` : `agrup-sense-dia-nom`;
