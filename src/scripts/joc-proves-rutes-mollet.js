...
import { MOLLET_CENTRE_RODONA } from './utils/punts-sobre-carrer.js';
...
<body>
  <div id="map"></div>
  <div id="panel">
    <h1>Rutes (joc de proves Mollet ~20 km)</h1>
...
async function main() {
  ...
  let entregues = construeixDatasetEntregues();

  console.log(`Magatzem (centre Mollet): ${MAGATZEM.x}, ${MAGATZEM.y}`);
  console.log(
    `Dataset: ${entregues.length} entregues · ${TOTAL_REF_PEDIDOS} referències de pedido (REF-PED-001…${padRefPedido(TOTAL_REF_PEDIDOS)})`,
  );
  console.log(`Mode resolució adreces sense coords inicials: ${args.geocodeOsm ? 'Nominatim' : 'anell determinista ' + DISTANCIA_ANELL_KM + ' km'}`);
  if (args.fast) {
    console.warn(
  ...
  for (const e of entregues) {
    const d = distanciaKmHaversine(MAGATZEM.x, MAGATZEM.y, e.coordenades.x, e.coordenades.y);
  ...
  const payloadComplet = {
    ...
    magatzem: MAGATZEM,
    distanciaAnellKm: DISTANCIA_ANELL_KM,
    dataset: {
      totalEntregues: entregues.length,
      totalReferenciesPedido: TOTAL_REF_PEDIDOS,
      pedidosPerEntrega: PEDIDOS_PER_ENTREGA,
